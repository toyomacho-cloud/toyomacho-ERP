...
const Settings = () => {
    ...
    const findDuplicateSKUs = () => {
        ...
        products.forEach(p => {
            if (p.sku) {
                ...
                if (!skuProducts[p.sku]) skuProducts[p.sku] = [];
                skuProducts[p.sku].push({
                    id: p.id,
        ...
        const duplicates = Object.entries(skuCount)
            .filter(([sku, count]) => count > 1)
            .map(([sku, count]) => ({
                sku,
                count,
                products: skuProducts[sku]
            }))
    ...
    // Function to identify products to delete (all duplicates, keeping most recent)
    const identifyProductsToDelete = () => {
        const toDelete = [];

        duplicateSKUs.forEach(dup => {
            // Sort all products by createdAt descending (most recent first)
    ...
    const executeDeleteDuplicates = async () => {
        ...
        try {
            ...
            alert(`✅ Se eliminaron ${productsToDelete.length} productos duplicados`);
                ...
                <div className="glass-panel" style={{ padding: '2rem', borderRadius: 'var(--radius-lg)' }}>
                    ...
                    <div style={{
                        ...
                        <p style={{ color: 'var(--text-secondary)', marginBottom: '1rem', fontSize: '0.875rem' }}>
                            Busca productos con el mismo SKU para limpiar tu inventario.
                        </p>
                        <button
                            className="btn btn-danger"
                            onClick={findDuplicateSKUs}
                            style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}
                        >
                            🔎 Buscar Duplicados
                        </button>
                            ...
                            <div style={{ marginTop: '1.5rem' }}>
                                {duplicateSKUs.length === 0 ? (
                                    <p style={{ color: 'var(--success)', fontWeight: 500 }}>
                                        ✅ No se encontraron SKUs duplicados. ¡Tu inventario está limpio!
                                    </p>
                                ) : (
                                    <>
                                        <p style={{ color: 'var(--danger)', fontWeight: 500, marginBottom: '1rem' }}>
                                            ⚠️ Se encontraron {duplicateSKUs.length} SKUs duplicados:
                                        </p>
                                                    ...
                                                    {duplicateSKUs.map((dup, idx) => (
                                                        <tr key={idx} style={{ borderBottom: '1px solid var(--border-color)' }}>
                                                            ...
                                                            <td style={{ padding: '0.75rem' }}>
                                                                {dup.products.map((p, i) => (
                                                                    <div key={i} style={{ fontSize: '0.8rem', marginBottom: '0.25rem' }}>
                                                                        • {p.description} ({p.brand || 'Sin marca'}) - Stock: {p.quantity}
                                                                    </div>
                                                                ))}
                                                            </td>
                                                        </tr>
                                        ...
                                        <button
                                            onClick={identifyProductsToDelete}
                                            className="btn btn-danger"
                                            style={{ marginTop: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}
                                        >
                                            🗑️ Limpiar Duplicados (conservar más reciente)
                                        </button>
                                    </>
                                )}
                            </div>
                    ...
                    <div className="glass-panel" style={{
                        ...
                        {productsToDelete.length === 0 ? (
                            <p style={{ color: 'var(--success)' }}>
                                ✅ No hay productos duplicados con stock 0 para eliminar.
                            </p>
                        ) : (
                            <>
                                <p style={{ marginBottom: '1rem', color: 'var(--text-secondary)' }}>
                                    Se eliminarán <strong style={{ color: 'var(--danger)' }}>{productsToDelete.length}</strong> productos con stock 0:
                                </p>
                                <div style={{ maxHeight: '300px', overflowY: 'auto', marginBottom: '1.5rem' }}>
                                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
                                        ...
                                        <tbody>
                                            {productsToDelete.map((p, idx) => (
                                                <tr key={idx} style={{ borderBottom: '1px solid var(--border-color)' }}>
                                                    <td style={{ padding: '0.5rem', fontFamily: 'monospace' }}>{p.sku}</td>
                                                    <td style={{ padding: '0.5rem' }}>{p.description}</td>
                                                    <td style={{ padding: '0.5rem', textAlign: 'center', color: 'var(--danger)' }}>{p.quantity}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                        ...
                        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
                            <button
                                onClick={() => { setShowDeleteConfirm(false); setProductsToDelete([]); }}
                            ...
                            {productsToDelete.length > 0 && (
                                <button
                                    onClick={executeDeleteDuplicates}
                                    className="btn btn-danger"
                                    disabled={deletingDuplicates}
                                >
                                    {deletingDuplicates ? 'Eliminando...' : `🗑️ Eliminar ${productsToDelete.length} productos`}
                                </button>
                            )}
                        </div>
                    </div>
                                            ...
                                            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                                                {user.id !== currentUser?.uid && (
                                                    <>
                                                        <button
                                                            onClick={() => {
                                                                setEditingModulesUser(user);
                                                                setTempModules(user.modules || { ...DEFAULT_MODULES });
                                                            }}
                                                            className="btn btn-primary"
                ...
                <div className="glass-panel" style={{ padding: '2rem', borderRadius: 'var(--radius-lg)' }}>
                    ...
                    {editingModulesUser && (
                        <UserPermissionsModal
                            user={editingModulesUser}
                            onClose={() => setEditingModulesUser(null)}
                            onSave={() => setEditingModulesUser(null)}
                        />
                    )}
                </div>
            )}
