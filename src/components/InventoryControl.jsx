...
const InventoryControl = () => {
    const { movements, approveMovement, rejectMovement, categories = [], updateCategory, deleteCategory } = useInventoryContext();
                                ...
                                {pendingMovements.map(m => (
                                    <tr key={m.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                                        ...
                                        <td style={{ padding: '0.75rem' }}>
                                            <div style={{ fontWeight: 'bold' }}>{m.sku}</div>
                                            <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>{m.productName}</div>
                                        </td>
                            ...
                            {filteredMovements.length > 0 ? (
                                filteredMovements.map((m) => (
                                    <tr key={m.id} style={{ borderBottom: '1px solid var(--border-color)', fontSize: '0.85rem' }}>
                                        ...
                                        <td style={{ padding: '0.75rem', fontFamily: 'monospace' }}>{m.sku}</td>
                                        <td style={{ padding: '0.75rem', fontWeight: 500 }}>{m.productName}</td>
                                        <td style={{ padding: '0.75rem', fontWeight: 'bold', textAlign: 'center' }}>
                                            {m.type === 'Salida' ? '-' : m.type === 'Entrada' ? '+' : ''}{m.quantity}
                                        </td>
                                        <td style={{ padding: '0.75rem', textAlign: 'center' }}>{m.newQty}</td>
                                        <td style={{ padding: '0.75rem' }}>{m.location}</td>
                                        <td style={{ padding: '0.75rem' }}>
                                            <span style={{
